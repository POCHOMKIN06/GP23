// Lerp_Sample.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "framework.h"
#include "Lerp_Sample.h"
...
tRootData root[] = {
	...
	{{100,200},{400,300},0,80},
	{{400,300},{250,150},80,200},
	...
	LPARAM lParam)
{
	...
	case WM_TIMER:
	{
		...
		g_pos.x =
			...
			(root[g_nRootIdx].end.x - root[g_nRootIdx].start.x) * deltaT;
		g_pos.y =
			root[g_nRootIdx].start.y +
			(root[g_nRootIdx].end.y - root[g_nRootIdx].start.y) * deltaT;
	...
	case WM_KEYDOWN:
		...
		case VK_ESCAPE:
		{
			int ret = MessageBox(hWnd, TEXT("終了しますか？"), TEXT("終了確認"), MB_YESNO);
			if (ret == IDYES) {
				PostQuitMessage(0);
			}
			break;
	...
	case WM_PAINT:
	{
		...
		wsprintf(aStr, "座標(x,y):%d, %d", g_pos.x, g_pos.y);
		TextOut(hDC, 300, 10, aStr, (int)strlen(aStr));


		// 線を描画
		for (int i = 0; i < sizeof(root) / sizeof(tRootData) - 1; i++)
		{
			MoveToEx(hDC, root[i].start.x, root[i].start.y, NULL);
			LineTo(hDC, root[i].end.x, root[i].end.y);
		}
